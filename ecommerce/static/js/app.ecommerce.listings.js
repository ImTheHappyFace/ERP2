app.controller('businessManagement.ecommerce.listings.item', function($scope, $http, $aside, $state, Flash, $users, $filter, $permissions, $sce) {
  $http({
    method: 'GET',
    url: '/api/ecommerce/insight/?mode=operations&listing=' + $scope.data.pk
  }).
      ...
      $scope.buildForm = function() {
        ...
        for (var i = 0; i < fields.length; i++) {
          if (fields[i].fieldType == 'boolean') {
            ...
            $http({
              method: 'GET',
              url: '/api/ecommerce/choiceLabel/?name=' + fields[i].unit
            }).
            then((function(i) {
        ...
        if (action == 'edit') {
          for (var i = 0; i < $scope.data.tableData.length; i++) {
            if ($scope.data.tableData[i].pk == parseInt(target)) {
              $scope.addTab({
                title: 'Edit Listing : ' + $scope.data.tableData[i].title,
                cancel: true,
                app: 'editListing',
                data: {
                  pk: target
